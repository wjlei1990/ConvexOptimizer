...
double FuncWrapper::valueAt(const vector<double>& point){
  if(point.size() != dim){
...
double FuncWrapper::finite_diff_grad(const vector<double> &point, int idim, double dx){
...
vector<double> FuncWrapper::gradientAt(const vector<double> &point){
  double dx = 0.001;
